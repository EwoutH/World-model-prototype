...
behaviors_readmes = glob.glob(os.path.join(os.path.dirname(__file__), '../../src/behaviors/**/README.md'), recursive=True)
policy_docs = glob.glob(os.path.join(os.path.dirname(__file__), '../../policy/**/*.md'), recursive=True)

# Define the toctree with the dynamically generated list of README.md files
toctree_behaviors = ['src/behaviors/' + os.path.relpath(readme, os.path.dirname(__file__)) for readme in behaviors_readmes]
toctree_policy = ['policy/' + os.path.relpath(doc, os.path.dirname(__file__)) for doc in policy_docs]
